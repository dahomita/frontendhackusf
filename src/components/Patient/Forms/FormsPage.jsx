...
const FormsPage = () => {
  ...
  const fetchForms = async () => {
    ...
    try {
      const response = await fetch(`${process.env.API_BASE_URL}/form/me`, {
        credentials: 'include',
      });
      ...
      setError(err.message || 'Failed to load your forms. Please try again.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="forms-page">
      <header className="forms-header">
        <div className="forms-title-section">
          <h1>My Messages</h1>
          <p className="forms-subtitle">Communicate with your nurse about non-urgent matters</p>
        </div>
        <button 
          className="new-form-button"
          onClick={() => navigate('/patient/forms/new')}
        >
          New Message
        </button>
      </header>
